"""
字符级语言模型训练脚本，支持 Transformer 单向语言模型，含 PPL 计算与文本生成。
用法:
...
class LM(nn.Module):
    def __init__(self, vocab_size, embed_dim, hidden_dim, num_layers, dropout, nhead=4, max_len=512):
        super().__init__()
    ...
    def forward(self, x):
        ...
        e = self.drop(self.embed(x) + self.pos_embed(pos))
        
        # 生成单向注意力掩码
        causal_mask = self.causal_mask[:T, :T]
        out = self.transformer(tgt=e, memory=e, tgt_mask=causal_mask)
...
@torch.no_grad()
def generate(model, char2idx, idx2char, start_text, max_len=200, device="cpu"):
    model.eval()
...
def main():
    ...
    parser.add_argument("--nhead",      type=int,   default=4,     help="Transformer 注意力头数")
    parser.add_argument("--dropout",    type=float, default=0.3)
    parser.add_argument("--lr",         type=float, default=1e-3)
    parser.add_argument("--val_ratio",  type=float, default=0.05)
    parser.add_argument("--corpus",     default="*.txt")
    parser.add_argument("--save",       default="best_model.pt")
    parser.add_argument("--start_text", default="故", help="生成文本的起始字符")
    parser.add_argument("--gen_len",    type=int,   default=200, help="生成文本的最大长度")
    ...
    generated = generate(
        model, checkpoint["char2idx"], checkpoint["idx2char"],
        start_text=args.start_text, max_len=args.gen_len, device=device
    )
    print(f"\n【生成结果】\n{generated}")
