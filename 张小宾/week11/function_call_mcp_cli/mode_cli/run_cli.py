...
NAMED_COMMANDS = {
    ...
    "location": {
        "argv": FINCLI_ARGV + ["location"],
        "arg_map": {"city": "--city"},
    },
    "weather_by_coord": {
        "argv": FINCLI_ARGV + ["weather-by-coord"],
        "arg_map": {"latitude": "--latitude", "longitude": "--longitude"},
    },
}
                ...
                "properties": {
                    "command": {
                        ...
                        "description": "rag_search（查年报，需 query+可选 stock_code/year/top_k）/"
                                       ...
                                       " weather（一步查天气，需 city）/"
                                       " location（获取坐标，需 city）/"
                                       " weather_by_coord（按坐标查天气，需 latitude/longitude）",
                    },
                    "args": {
                        "type": "object",
                        "description": "命令参数。rag_search: {query, stock_code?, year?, top_k?}; weather: {city}; location: {city}; weather_by_coord: {latitude, longitude}",
                    },
                },
                "required": ["command"],
            },
...
SYSTEM_PROMPT_NAMED = (
    ...
    "查天气必须先 run_cli(command='location', args={city:'南京'}) 获取坐标，然后 run_cli(command='weather_by_coord', args={latitude:xxx, longitude:xxx}) 查询天气。"
...
SYSTEM_PROMPT_BASH = (
    ...
    "查天气必须分两步：先 fincli location --city 南京 获取坐标，然后 fincli weather-by-coord --latitude xxx --longitude xxx 查询天气。"
