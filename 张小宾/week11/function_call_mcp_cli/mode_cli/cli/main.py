...
from src.weather_backend import get_weather, getLocation, getWeather as get_weather_by_coord  # noqa: E402
...
def main():
    ...
    # fincli weather ...
    p_weather = sub.add_parser("weather", help="查询城市天气（一步完成）")
    p_weather.add_argument("--city", required=True, help="城市中文名，如 宁德")

    # fincli location ...
    p_location = sub.add_parser("location", help="获取城市地理位置坐标")
    p_location.add_argument("--city", required=True, help="城市中文名，如 宁德")

    # fincli weather-by-coord ...
    p_weather_coord = sub.add_parser("weather-by-coord", help="根据经纬度查询天气")
    p_weather_coord.add_argument("--latitude", type=float, required=True, help="纬度，如 26.6479")
    p_weather_coord.add_argument("--longitude", type=float, required=True, help="经度，如 119.3067")
    ...
    if args.cmd == "search":
        ...
        print(getLocation(args.city))
    elif args.cmd == "weather-by-coord":
        print(get_weather_by_coord(args.latitude, args.longitude))
