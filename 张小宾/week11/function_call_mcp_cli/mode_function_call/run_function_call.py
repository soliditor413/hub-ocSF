...
TOOLS_SCHEMA = [
    ...
    },
    {
        ...
        "function": {
            "name": "getLocation",
            "description": "根据城市名称获取地理位置坐标，返回 JSON 格式数据包含 latitude 和 longitude。必须先调用此工具获取坐标，然后才能调用 getWeather 查询天气。",
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function": {
            "name": "getWeather",
            "description": "根据经纬度坐标获取当前天气及未来3天预报。必须先调用 getLocation 获取坐标后才能调用此工具。",
            "parameters": {
                "type": "object",
                "properties": {
                    "latitude": {"type": "number", "description": "纬度，如 26.6479"},
                    "longitude": {"type": "number", "description": "经度，如 119.3067"},
                },
...
TOOL_DISPATCH = {
    ...
    "getWeather": get_weather_by_coord,
}
...
SYSTEM_PROMPT = (
    ...
    "涉及天气时，必须先调用 getLocation 获取城市坐标，然后再调用 getWeather 根据坐标查询天气。"
    "本回合你可以一次调用多个工具。"
)
...
def run(client, model: str, question: str, verbose: bool = True) -> dict:
    ...
    支持多轮工具调用：如果模型继续返回 tool_calls，会循环执行直到不再调用工具。
