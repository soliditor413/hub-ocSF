...
from src.weather_backend import get_weather as _get_weather, getLocation, getWeather as _getWeather  # noqa: E402
...
@mcp.tool()
def getLocation(city: str) -> str:
    """
    根据城市名称获取地理位置坐标。

    Args:
        city: 城市中文名，如 '宁德'。同名地名会自动取行政级别更高的（如福建宁德而非西藏宁德）。

    Returns:
        JSON 格式字符串，包含 city、country、admin1、latitude、longitude。
    """
...
def getWeather(latitude: float, longitude: float) -> str:
    """
    根据经纬度坐标获取当前天气及未来3天预报。

    Args:
        latitude: 纬度，如 26.6479。
        longitude: 经度，如 119.3067。

    Returns:
        包含温度、湿度、风速、天气状况和3天预报的文字描述。
    """
    return _getWeather(latitude, longitude)
