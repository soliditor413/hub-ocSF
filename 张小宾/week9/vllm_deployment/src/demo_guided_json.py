...
使用方式：
  ...
  # macOS（本地模式，不支持 guided_json）：
  python demo_guided_json.py --local
"""
...
def run_generate(user_msg: str, mode: str, local_mode: bool = False) -> tuple[str, float]:
    global client
    if client is None:
    ...
    extra = {}
    kwargs = {}
    if mode == "guided_json" and not local_mode:
...
def main():
    ...
    parser.add_argument("--local", action="store_true",
                        help="macOS 本地模式（不支持 guided_json）")
    ...
    if args.local:
        print("\n⚠️  macOS 本地模式不支持 guided_json 约束解码\n")
    ...
    for user in TEST_CASES:
        ...
        for mode in ["raw", "response_format", "guided_json"]:
            out, _ = run_generate(user, mode, args.local)
            ev = evaluate(out)
            c = counters[mode]
