...
使用方式（需停掉 vLLM server 以释放显存）：
  ...
  # macOS（CPU 模式，不支持 vLLM）：
  python bench_throughput.py --cpu
...
# ── 配置 ──────────────────────────────────────────────────────────────
MODEL_PATH = "/Users/zhangxiaobin/.cache/modelscope/hub/models/Qwen/Qwen2-0___5B-Instruct"
N_PROMPTS = 50
MAX_NEW_TOKENS = 100
BATCH_SIZE = 8
QUICK_MODEL = "gpt2"
...
if sys.platform == "darwin":
    USE_CPU = True
print(f"Platform: {sys.platform}, Using {'CPU' if USE_CPU else 'CUDA'}")
...
def bench_transformers(prompts: list[str]) -> dict:
    ...
    if USE_QUICK or (USE_CPU and not os.path.exists(MODEL_PATH)):
        model_name = QUICK_MODEL
        print("\n" + "=" * 70)
        print(f"  加载 transformers {model_name}（快速模式）")
        print("=" * 70)
    else:
...
def bench_vllm(prompts: list[str]) -> dict:
    if USE_CPU:
        print("\n" + "=" * 70)
        print("  ⚠️  CPU 模式：跳过 vLLM 测试（vLLM 需要 CUDA）")
        print("=" * 70)
        return {"vllm": {"time": float("inf"), "gen_tokens": 0, "qps": 0, "tps": 0}}
...
def plot_results(r: dict, out_path: str):
    if USE_CPU:
        ...
        times = [r["serial"]["time"], r["batch"]["time"]]
        qps = [r["serial"]["qps"], r["batch"]["qps"]]
        tps = [r["serial"]["tps"], r["batch"]["tps"]]
        colors = ["#aab7c4", "#82b1ff"]
        title_suffix = " (CPU Mode)"
    else:
    ...
    plt.suptitle(f"vLLM vs Transformers: Throughput Benchmark (Qwen2-0.5B){title_suffix}",
...
def main():
    ...
    parser.add_argument("--quick", action="store_true", help="使用 gpt2 快速模式（跳过 Qwen2-0.5B）")
    args = parser.parse_args()

    global USE_CPU, USE_QUICK
    if args.cpu:
        USE_CPU = True
    if args.quick:
        USE_QUICK = True
    ...
    print(f"  Mode: {'CPU' if USE_CPU else 'CUDA'} {'(Quick)' if USE_QUICK else ''}")
    print("=" * 70)
    ...
    print("  核心结论：")
    if USE_CPU:
        print(f"    CPU 模式：transformers batch 相对串行加速：{results['batch']['qps']/results['serial']['qps']:.1f}×")
        print("    ⚠️  vLLM 需要 CUDA，无法在 CPU 上运行")
        print("    建议在 Linux + NVIDIA GPU 环境运行完整 benchmark")
    else:
