...
def _setup_cn_font():
        ...
        for d in ["/System/Library/Fonts", "/Library/Fonts"]:
            if os.path.isdir(d):
                for name in os.listdir(d):
                    if any(k in name.lower() for k in ("pingfang","heiti","sthei","song","kaiti","hiragino")):
                        candidate_paths.append(os.path.join(d, name))
    elif _platform.system() == "Windows":
        candidate_paths = ["C:/Windows/Fonts/simhei.ttf","C:/Windows/Fonts/msyh.ttc","C:/Windows/Fonts/simsun.ttc"]
    else:
        candidate_paths = ["/usr/share/fonts/opentype/noto/NotoSansCJK-Regular.ttc","/usr/share/fonts/truetype/wqy/wqy-microhei.ttc"]
    picked = None
    ...
    if picked:
        try:
            _fm.fontManager.addfont(picked)
            fp = _fm.FontProperties(fname=picked)
            fname = fp.get_name()
            plt.rcParams["font.sans-serif"] = [fname] + ["PingFang SC","Heiti SC","STHeiti","SimHei","Microsoft YaHei","Noto Sans CJK SC","Arial Unicode MS"]
            plt.rcParams["font.family"] = "sans-serif"
            plt.rcParams["axes.unicode_minus"] = False
            print(f"[font] 已设置中文字体: {fname}")
            return
        except Exception:
            pass
    plt.rcParams["font.sans-serif"] = ["PingFang SC","PingFang HK","Heiti SC","STHeiti","SimHei","Microsoft YaHei","Noto Sans CJK SC","WenQuanYi Zen Hei","Arial Unicode MS"]
    plt.rcParams["font.family"] = "sans-serif"
    plt.rcParams["axes.unicode_minus"] = False
