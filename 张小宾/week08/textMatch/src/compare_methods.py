...
def load_and_eval(method, tokenizer, device, split, batch_size, ckpt_dir, data_dir):
...
def parse_args():
    ...
    parser.add_argument("--data_dir",    default=str(DATA_DIR), type=str,
                        help="数据集目录")
    parser.add_argument("--output_dir",  default=str(ROOT / "outputs"), type=str,
                        help="输出目录（checkpoints 从此读取，figures/logs 保存于此）")
