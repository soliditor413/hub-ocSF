...
CKPT_DIR  = ROOT / "作业outputs" / "checkpoints"
FIG_DIR   = ROOT / "作业outputs" / "figures"
...
【1】数据层面
  ...
  │
  ├─ 数据增强（正样本扩充）
  │    BQ Corpus train 约 34K 正样本对（占比约 50%），相比 AFQMC 已较充足，
  │    但难判例仍可进一步增强。
  │    → 对正样本做同义改写（换词、调序），扩充正例数量。
  │      可用 LLM API 批量生成改写句。
  │
  └─ 跨数据集迁移
       LCQMC 等开放问句匹配数据集包含更多样的句对。
       → 先在 LCQMC（238K 对）上预训练 Sentence-BERT，再 fine-tune 到 BQ Corpus，
         利用大数据集的语义泛化能力。
""")
...
""")
  ...
  └─ 换用金融领域预训练模型
       ...
       BQ Corpus 属于银行金融领域，很多错误源于领域术语理解不准确。
""")
...
""")
    ...
    print("""【4】评估与部署层面
  ├─ 阈值校准
  │    当前阈值在 val 集上网格搜索，但 val 正负比（约 50:50）和线上真实分布可能不同。
