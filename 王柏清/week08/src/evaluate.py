...
教学重点：
  ...
  2. 阈值搜索 (threshold search)：在验证集上枚举阈值，选 accuracy 最高的
...
import sys
# Windows 默认控制台编码为 GBK，无法输出特殊字符，强制 stdout 用 UTF-8
...
def eval_biencoder(model, loader, device, find_threshold=True, threshold=0.5):
    ...
    # AUC：若某个 split 的 labels 只有一类则无法计算，跳过（BQ Corpus 各 split 均含正负两类）
    try:
...
def _find_best_threshold(sims, labels):
    """枚举 [0.0, 1.0] 区间 101 个候选阈值，返回使 accuracy 最高的那个。"""
...
def _setup_cn_font():
    """将系统中文字体设为 matplotlib 默认，避免中文显示成方框 □；并修正坐标轴负号显示。

    matplotlib 默认字体（DejaVu Sans）不含中文字形，标题/图例/坐标轴里的中文会变成 □。
    这里挑一个系统中文字体（黑体/雅黑/宋体/Noto）注册为默认 sans-serif，全局生效一次即可。
    """
    global _CN_FONT_READY
    if _CN_FONT_READY:
        return
    # 按"字体名"而非文件名匹配，并按优先级挑选——必须含常用汉字。
    # 注意 SimSun-ExtB(simsunb.ttf) 只含生僻字，绝不能选，否则常用字仍是 □。
    ...
    for name in preferred:
        if name in available:
            ...
            break
    # 中文字体的 U+2212 减号字形常缺失，关掉后用 ASCII '-'，避免负号也变成 □
...
def parse_args():
    ...
    parser.add_argument("--split",      default="validation", choices=["validation", "test"],
                        help="评估的数据集分割（BQ Corpus 的 validation 与 test 均含标签，皆可用于评估）")
