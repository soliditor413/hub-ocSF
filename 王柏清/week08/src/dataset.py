...
class TripletDataset(Dataset):
    ...
    构建逻辑（离线静态构建）：
      ...
      → 最终 triplet 数量 ≈ 正样本对数量（BQ Corpus train 约 34K 条）
...
def build_pair_loaders(data_dir, tokenizer, max_length=64, batch_size=32):
    ...
    注意：BQ Corpus 三个 split 均含真实标签且正负基本均衡，validation 与 test 都可用于评估。
    """
    ...
    print(f"  test  : {len(test_ds):>7,} 条, {len(test_loader):>5} batch  (BQ Corpus test 含标签，可用于评估)")
