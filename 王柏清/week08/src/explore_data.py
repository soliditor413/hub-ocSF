"""
BQ Corpus（银行金融领域问句匹配）数据集探索与可视化

教学重点：
  1. 文本匹配数据的结构——sentence pair + binary label，与分类任务的本质区别
  2. 类别基本均衡（正/负例各约 50%）——与 AFQMC（~31% 正例）不同，BQ Corpus
     在构造时已做正负平衡，因此无需类别加权
...
使用方式：
  ...
  python explore_data.py --data_dir ../data/bq_corpus --output_dir ../作业outputs/figures
...
import sys
# Windows 默认控制台编码为 GBK，无法输出 ✓/✗/├─ 等字符，强制 stdout 用 UTF-8
...
OUTPUT_DIR = ROOT / "作业outputs" / "figures"

# 中文字体（matplotlib 默认不支持中文，会把中文显示成方框 □）
_CN_FONT = None
def _get_font():
    """按"字体名"优先级挑一个含常用汉字的系统中文字体，返回 FontProperties。

    注意：不能按文件名子串匹配——simsunb.ttf(SimSun-ExtB) 只含生僻字，
    会被误选导致常用汉字仍是 □。这里按真实字体名匹配并按优先级挑选。
    """
    global _CN_FONT
    if _CN_FONT is None:
        ...
        for p in fm.findSystemFonts():
            try:
                available.setdefault(fm.FontProperties(fname=p).get_name(), p)
            except Exception:
                continue
        chosen = next((available[n] for n in preferred if n in available), None)
        _CN_FONT = fm.FontProperties(fname=chosen) if chosen else fm.FontProperties()
        # 负号字形常缺失，关掉避免坐标轴负号也变 □（本图多为非负值，加上更稳）
...
def plot_label_distribution(splits_data, output_dir):
    ...
    for ax, (split_name, rows) in zip(axes, splits_data.items()):
        ...
        ax.set_ylim(0, max(counts) * 1.18)          # 顶部留白，避免柱顶标注与标题重叠
        ...
        for lbl in ax.get_xticklabels():            # x 轴中文刻度也套用中文字体，否则仍是 □
            lbl.set_fontproperties(fp)

    # 动态计算整体正例占比，避免硬编码（BQ Corpus 正负基本均衡，各约 50%）
    all_labels = [r["label"] for rows in splits_data.values() for r in rows]
    pos_ratio  = sum(1 for l in all_labels if l == 1) / max(len(all_labels), 1) * 100
    fig.suptitle(f"BQ Corpus 标签分布（正例约 {pos_ratio:.0f}%，正负基本均衡）",
                 fontproperties=fp, fontsize=12, y=1.02)
    fig.tight_layout()
...
def print_stats(name, rows):
    ...
    if n_unlabeled:
        # 兼容标签未公开的竞赛格式（如 AFQMC test 用 -1）；BQ Corpus 三个 split 均有真实标签
        print(f"  标签未公开（竞赛格式）: {n_unlabeled:>6,} 条  —— 仅供参考，不用于评估")
    else:
...
def parse_args():
    parser = argparse.ArgumentParser(description="BQ Corpus 数据集探索")
