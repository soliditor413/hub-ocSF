...
DASHSCOPE_URL = "https://api.deepseek.com/v1"
...
def parse_args():
    ...
    parser.add_argument("--num_samples", default=100, type=int,
                        help="评估样本数（默认 100，全集太贵；BQ Corpus val 共 8620 条）")
...
def main():
    ...
    # 随机采样（保持正负比例；BQ Corpus 正负约 50:50，故取约一半正样本）
    pos_rows = [r for r in rows if r["label"] == 1]
    neg_rows = [r for r in rows if r["label"] == 0]
    n_pos = min(args.num_samples // 2, len(pos_rows))   # 约 1/2 正样本（数据集均衡）
