...
使用方式：
  ...
  python train_sft.py --num_train -1         # LoRA，全部约 69K 条（BQ Corpus 平衡采样后，正负各约 34K）
...
MODEL_PATH = ROOT / "Qwen2-0.5B-Instruct"
...
# 本地 Qwen2-0.5B-Instruct 目录缺 tokenizer_config.json（chat_template 未设置），
# 这里内置官方 Qwen2 ChatML 模板作为兜底：加载 tokenizer 后若缺失则补上。
QWEN2_CHAT_TEMPLATE = (
...
def parse_args():
    ...
    parser.add_argument("--num_train",   default=5000, type=int,
                        help="训练样本数，-1 使用全部（BQ Corpus 平衡采样后约 69K 条）")
...
def main():
    ...
    # ── 正负平衡采样（BQ Corpus 正负本就约 50:50，这里确保严格平衡并按 num_train 控制数量）──────
    ...
    if tokenizer.chat_template is None:        # 本地目录缺 chat_template，补上 Qwen2 模板
    ...
    model = AutoModelForCausalLM.from_pretrained(
        ...
        torch_dtype=torch.float32,  # transformers 4.x 用 torch_dtype=（5.x 才改名为 dtype=）
