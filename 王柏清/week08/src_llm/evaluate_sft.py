...
使用方式：
  ...
  python evaluate_sft.py --ckpt_dir ../作业outputs/sft_full_ckpt  # 评估全量微调
...
ADAPTER_DIR = ROOT / "作业outputs" / "sft_adapter"
LOG_DIR     = ROOT / "作业outputs" / "logs"
...
def main():
    ...
    if not ckpt_dir.exists():
        ...
        print("  LoRA（默认）保存到:   作业outputs/sft_adapter/")
        print("  全量微调保存到:        作业outputs/sft_full_ckpt/")
        return
    ...
    print(f"""
多方对比（BQ Corpus validation 集，所有方案均使用 Accuracy + F1，直接可比）
  ┌──────────────────────────────────────────┬──────────┬──────────┐
  │ 方法                                     │ Accuracy │ F1(pos)  │
  ├──────────────────────────────────────────┼──────────┼──────────┤
