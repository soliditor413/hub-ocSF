...
class TorchLSTM(nn.Module):
    def __init__(self, vector_dim, sentence_length, vocab, hidden_size):
        ...
        self.lstm = nn.LSTM(vector_dim, hidden_size, batch_first=True)        #RNN层
    ...
    def forward(self, x, y=None):
        ...
        lstm_out, _ = self.lstm(x)                   # (batch, sen_len, hidden)
...
def main():
    ...
    vocab = build_vocab()
    # 建立模型
    model = TorchLSTM(char_dim, sentence_length, vocab, hidden_size=16)
    # 选择优化器
...
def predict(model_path, vocab_path, input_strings):
    ...
    model = TorchLSTM(char_dim, sentence_length, vocab, hidden_size=16)     #建立模型
